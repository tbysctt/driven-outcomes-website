...
export function GradientBorderButton({
  ...
  const hoverClasses = backgroundColor
    ? ""
    : "hover:bg-gradient-to-r hover:from-primary-brand-500 hover:to-secondary-brand-500";
  ...
  const getBackgroundImage = () => {
    if (backgroundColor) {
      ...
      if (backgroundColor.includes("white/10")) {
        return "linear-gradient(rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.1)), linear-gradient(to right, var(--color-primary-brand-500), var(--color-secondary-brand-500))";
      }
      if (backgroundColor.includes("white/20")) {
        return "linear-gradient(rgba(255, 255, 255, 0.2), rgba(255, 255, 255, 0.2)), linear-gradient(to right, var(--color-primary-brand-500), var(--color-secondary-brand-500))";
      }
      // Default: try to use the backgroundColor class and add gradient
      return `linear-gradient(var(--tw-gradient-stops)), linear-gradient(to right, var(--color-primary-brand-500), var(--color-secondary-brand-500))`;
    }
    return "linear-gradient(transparent, transparent), linear-gradient(to right, var(--color-primary-brand-500), var(--color-secondary-brand-500))";
  };
