...
<div class="container mx-auto space-y-24 lg:space-y-32">
    <header class="mb-8">
        <h1 class="text-5xl font-medium tracking-tight text-zinc-950 sm:text-6xl mb-4">
            <?php _e('Workshops', 'driven-outcomes'); ?>
        </h1>
    ...
    <?php if (have_posts()): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php while (have_posts()): the_post(); ?>
                <article id="post-<?php the_ID(); ?>" <?php post_class('group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-zinc-100 hover:shadow-md transition-shadow'); ?>>
                    ...
                    <div class="flex flex-col flex-1 p-6">
                        ...
                        <a href="<?php the_permalink(); ?>" class="inline-flex items-center text-sm font-semibold text-zinc-950 hover:text-zinc-600 transition-colors">
                            <?php _e('Learn more', 'driven-outcomes'); ?>
    ...
    <?php else: ?>
        <div class="text-center py-12">
            <p class="text-lg text-zinc-600"><?php _e('No workshops found. Check back soon!', 'driven-outcomes'); ?></p>
        </div>
    <?php endif; ?>
</div>
